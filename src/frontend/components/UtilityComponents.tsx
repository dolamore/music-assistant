...
export const ControlsContainer = observer(
  ({
    ...
    defaultValue,
    label,
  }: ControlsContainerInputType): ReactElement => {
    return (
      <div className={`${id}-controls-container controls-container container`}>
        <label htmlFor={`${id}-input`}>{label}</label>
        <div id={`${id}-controls`} className={`${id}-controls container`}>
          <ChangingButton
            id={`${id}-decrease-5`}
            label="-5"
...
export const ChangingButton = observer(
  ({ id, onClick, disabled, label }: ChangingButtonInputType): ReactElement => (
    <button
      id={id}
      onClick={onClick}
      disabled={disabled}
...
export const InputField = observer(
  ({
    ...
    minLimit,
    maxLimit,
  }: InputFieldInputType): ReactElement => {
...
export const ToggleCheckbox = observer(
  ({ id, checked, onChange, label }: ToggleCheckboxInputType): ReactElement => {
    return (
      <label>
