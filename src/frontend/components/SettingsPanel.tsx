...
export default observer(function SettingsPanel({
  metronomeManager,
}: MetronomeManagerInputType): ReactElement {
  const indices: number[] = metronomeManager.beatBarsManager.beats.map(
    (_, i: number): number => i,
  );
  ...
  return (
    ...
    >
      ...
      <div
        className="sound-settings-grid"
        style={{ gridTemplateColumns: `150px repeat(${numColumns}, 1fr)` }}
      >
        <div className="sounds-settings-labels">
          <span>Beat</span>
          <span>Oscillator</span>
          {DEFAULT_SOUND_SETTINGS.map(
            (setting: SoundSetting): ReactElement => (
...
const SoundRow = observer(({ metronomeManager, index }: SoundRowInputType) => {
  ...
  return (
    <div className="sound-row">
      ...
      {beat.beatSound.instrument.soundSettings.map(
        (setting: SoundSetting): ReactElement => (
          <InputField
            key={setting.key}
            id={`${setting.key}-${index}`}
            inputVar={Number(setting.value)}
            changeHandler={beat.beatSound.instrument.updateSoundSetting.bind(
              beat.beatSound.instrument,
              setting.key,
            )}
