...
import {
  ...
  TimeSignatureInfoInputType,
} from "../models/ComponentsTypes";

export default observer(function TimeSignatureControls({
  metronomeManager,
}: MetronomeManagerInputType) {
  return (
    <div className="time-signature-controls-container container">
      ...
      <TimeSignatureInfo
        timeSignature={metronomeManager.beatBarsManager.timeSignature}
      />
...
const BeatsControlGroup = observer(
  ({ metronomeManager }: MetronomeManagerInputType): ReactElement => {
    const increaseBeats = () => {
      ...
      <div className="time-signature-control-group">
        ...
        <div>
          <ChangingButton
            ...
            disabled={
              metronomeManager.beatBarsManager.beats.length >= MAX_BEATS_AMOUNT
            }
            label="+"
          />
          <span id="beats-counter">
            {metronomeManager.beatBarsManager.beats.length}
          </span>
          <ChangingButton
            id="decrease-beats-button"
            onClick={decreaseBeats}
            disabled={
              metronomeManager.beatBarsManager.beats.length <= MIN_BEATS_AMOUNT
            }
            label="-"
          />
        </div>
...
const NotesControlGroup = observer(
  ({ metronomeManager }: MetronomeManagerInputType): ReactElement => {
    const decreaseNotes = () => {
      ...
      <div className="time-signature-control-group">
        ...
        <div>
          <ChangingButton
            ...
            disabled={metronomeManager.beatBarsManager.beats.some(
              (beat) => beat.noteSettings.noteSize === 64,
            )}
            label="+"
          />
          <ChangingButton
            ...
            disabled={metronomeManager.beatBarsManager.beats.some(
              (beat) => beat.noteSettings.noteSize === 1,
            )}
            label="-"
          />
        </div>
...
const TimeSignatureInfo = observer(
  ({ timeSignature }: TimeSignatureInfoInputType): ReactElement => {
    return (
