...
describe('AudioContextProvider', () => {
    const TestComponent = () => {
        React.useEffect(() => {
            if (Tone.getContext().state === 'suspended') {
                console.error(new Error("The AudioContext is 'suspended'. Invoke Tone.start() from a user action to start the audio."));
            }
        }, []);
    ...
    it('should not cause AudioContext error after users interaction with provider', async () => {
        render(
    ...
    it('should delete listeners after the first click', async () => {
        ...
        const initialRemoveEventCount = removeEventListenerSpy.mock.calls.length;

        await act(async () => {
            fireEvent.click(screen.getByTestId('test-child'));
        });

        expect(removeEventListenerSpy.mock.calls.length - initialRemoveEventCount).toBe(5);
    ...
    it('should work with all the users interactions', async () => {
        const events = [
